...
def dictionaryFromJsonLinesFiles(fname):
	''' Create a dictionary object from a JsonLines file '''
	dictionary = corpora.Dictionary(d['words'] for d in loadSplitJsonLines(fname))
	return dictionary

def filterDictionary(dictionary):
	''' Filter stopwords and unique words from a dictionary object '''
...
def buildNewDictionary(fname = 'docs/sites.jl.', saveas = 'dict.dict'):
	''' Load and filter a dictionary from a JsonLines file, then save it '''
	dictionary = dictionaryFromJsonLinesFiles(fname)
